...
def get_reward(test_texts, all_len):
    rewards = []
    for single_test in test_texts:
        tmp_rewards = []
        pipe_outputs = rm_pipe(single_test, **pipe_kwargs)
        i = 0
        for output in pipe_outputs:
            tmp_rewards.append(output[0]['score']) #- all_len[i] * 0.001)
            i += 1
...
def change_of_format(prom, resp):
    ...
    if 'DS' in ds_dir or 'Deep' in ds_dir:
        ...
        resp_list = resp.split("ки\n")
    resp_list = [i.strip() for i in resp_list]
    text = resp_list[0]
    message = [{"role": "user", "content":prom}] + [{"role": "assistant", "content": text}]
    chat_list.append(rm_tokenizer.apply_chat_template(message, tokenize=False).replace(rm_tokenizer.bos_token, ""))
    for res in resp_list[1:]:
        text += " " + res
        message = [{"role": "user", "content":prom}] + [{"role": "assistant", "content": text}]
        chat_list.append(rm_tokenizer.apply_chat_template(message, tokenize=False).replace(rm_tokenizer.bos_token, ""))
